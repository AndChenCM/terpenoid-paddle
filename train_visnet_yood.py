...
def get_data_loader(mode, batch_size=256):
    ...
    if mode == 'train':
        data_list = pickle.load(open("work/train_semi_from_mol_clean.pkl", 'rb'))

        # Load validation SMILES
        validation_smiles = pd.read_csv('data/clean_valid_smiles5%.csv')['smiles'].tolist()
        
        # Split data_list into train and valid based on the SMILES in validation_smiles.csv
        train_data = [data for data in data_list if data['Smiles'] not in validation_smiles]
        valid_data = [data for data in data_list if data['Smiles'] in validation_smiles]

        print(f'len train is {len(train_data)}, len valid is {len(valid_data)}')

        # Create dataset instances
        train_ds = InMemoryDataset(train_data)
        valid_ds = InMemoryDataset(valid_data)

        # Create data loaders
        train_dl = train_ds.get_data_loader(batch_size=batch_size, shuffle=True, collate_fn=collate_fn)
        valid_dl = valid_ds.get_data_loader(batch_size=batch_size, shuffle=True, collate_fn=collate_fn)
...
def trial(model_version, batch_size, lr,  tmax, weight_decay, max_bearable_epoch, max_epoch,hidden_channels):
    ...
    # model_path = '/home/chenmingan/workplace/paddle/terpenoid-paddle/pretrain_weight/visnet_hs80_l6_rbf32_lm2_pt_on_train_mol_exhaust/visnet_hs80_l6_rbf32_lm2_pt_on_train_mol_exhaust11.pkl'
    ...
    lr = optimizer.lr.ReduceOnPlateau(lr, mode='min', factor=0.5, patience=10)
...
trial('visnet_hs80_l6_rbf32_lm2_bs32_lr1e-4_IDopt_smL1loss_rop_yood5%', batch_size, lr, tmax, weight_decay, max_bearable_epoch, max_epoch, hidden_channels)
