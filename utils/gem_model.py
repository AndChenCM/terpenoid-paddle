...
class GeoPredModel(nn.Layer):
    ...
    def __init__(self, model_config, compound_encoder):
        ...
        if 'Cm' in self.pretrain_tasks:
            ...
            self.Cm_linear = nn.Linear(compound_encoder.hidden_channels, self.Cm_vocab + 3)
            self.Cm_loss = nn.CrossEntropyLoss()
        # functinal group
        self.Fg_linear = nn.Linear(compound_encoder.hidden_channels, model_config['Fg_size']) # 494
        ...
        if 'Bar' in self.pretrain_tasks:
            self.Bar_mlp = MLP(2,
                    ...
                    act=self.act,
                    in_size=compound_encoder.hidden_channels * 3,
                    out_size=1,
        ...
        if 'Blr' in self.pretrain_tasks:
            self.Blr_mlp = MLP(2,
                    hidden_size=self.hidden_size,
                    act=self.act,
                    in_size=compound_encoder.hidden_channels * 2,
                    out_size=1,
                    dropout_rate=self.dropout_rate)
        ...
        if 'Adc' in self.pretrain_tasks:
            ...
            self.Adc_mlp = MLP(2,
                    hidden_size=self.hidden_size,
                    in_size=self.compound_encoder.hidden_channels * 2,
                    act=self.act,
    ...
    def forward(self, graph_dict, feed_dict, return_subloss=False):
        ...
        Build the network.
        """
        node_repr, _ = self.compound_encoder.forward(
                graph_dict['atom_bond_graph'])
        # import pdb; pdb.set_trace()
        masked_node_repr, __ = self.compound_encoder.forward(
                graph_dict['masked_atom_bond_graph'])
