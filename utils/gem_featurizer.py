...
from .compound_tools import Compound3DKit 
...
def mask_context_of_geognn_graph(
    ...
    #     if name == 'atom_pos':
    #         reshaped_feat = g.node_feat[name].reshape([-1, 3])
    #         noise = np.random.normal(0, 0.1, reshaped_feat[Cm_node_i].shape)
    #         reshaped_feat[Cm_node_i] += noise
    #         g.node_feat[name] = reshaped_feat.flatten()
    #         continue
    #     else:
