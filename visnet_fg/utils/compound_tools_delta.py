...
class Compound3DKit(object):
    ...
    @staticmethod
    def get_MMFF_atom_poses(mol, numConfs=None, return_energy=True):
...
def mol_to_geognn_graph_data(mol, atom_poses, energy ,dir_type):
    """
    mol: rdkit molecule
    ...
    data['label'] = np.array(energy, 'float32')
...
def mol_to_geognn_graph_data_MMFF3d(mol):
    ...
    if len(mol.GetAtoms()) <= 400:
        ...
        mol, atom_poses, energy = Compound3DKit.get_MMFF_atom_poses(mol, numConfs=10)
    else:
        atom_poses = Compound3DKit.get_2d_atom_poses(mol)
    #return mol_to_geognn_graph_data(mol, atom_poses, dir_type='HT')
    return mol_to_geognn_graph_data(mol, atom_poses, energy, dir_type='HT')
