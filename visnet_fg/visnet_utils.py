...
allowable_features = {
    ...
    'possible_daylight_fg_counts': list(range(1, 127)) + ['misc'],
...
def get_atom_feature_dims():
    return dict(
        ...
        daylight_fg_counts=len(allowable_features['possible_daylight_fg_counts']) + 1,
    )
...
class FgEncoder(nn.Layer):
    ...
    def forward(self, node_feat: dict):
        x_embedding = 0
        #import logging
        #logging.debug(print('node_feat',node_feat['daylight_fg_counts'].shape))
        for key in node_feat.keys():
            if key in self.feature_list:
